...
The starting values were sampled close to the peak-values of the target densities. We show here the algorithm targeting a bivariate Gaussian distribution in $10,000$ iterations - we start updating the covariance matrix after 1000 non-adaptive steps.
