...
\section{Introduction to Adaptive MCMC - the AM}
...
<<mh_on_correlated_guassian_plots,out.width='0.7\\linewidth',out.height='0.5\\linewidth',fig.align="center",fig.cap="The first (left) and second(right) components of the markov chain resulting from Metropolis-Hastings (MH) on a correlated 8-dimensional Gaussian.", echo=FALSE, cache=TRUE>>=
...
The usual empirical covariance matrix estimator used so far to performe the adaptive algorithms is optimal in the classical setting with large samples and fixed low dimensions. This estimator performs poorly in the high dimensional setting and in particular when the dimension of the parameter space is larger than the nnumber of observations. In the recent literature, sevaral alternative estimatation techiniques for the covariance matrix have been proposed. In this paper we will focus on two different regularization techniques: the shrinkage estimator and the Choleski-based regularization method.
...
Define a target identity matrix as $D=diag(d)$ where $d$ represents the dimension of the problem. The idea of shrinkage estimation of the covariance matrix is to take a weighhted avearge of the empirical covariance matrix and the target matrix. Thus, given a parameter $\lambda$ representing the shrinkage intensity, (for its computation see Sch\"afer and Strimmer (2005) )the covariance matrix used in the algorithm can be defined as:

$\hat{C} = (1-\lambda) \hat{\Sigma} + \lambda*\hat{D}$

Notice that $0 < \lambda< 1$. When  $\lambda=0$, no shrinkage is applied to the  sample covariance matrix and the sample covariance matrix estimatorm is then used. In constrast, the case of $\lambda=1$ indicates a complete shrinkage of all pairwise covariances. In this case we are ignoring the existence of any covariances among the random variables considered. It is possible to show that, under general conditions, there exists a shrinkage intensity for which the resulting shrinkage estimator contains less estimation error than the original sample estimator; see James and Stein (1961).

Shrinkage estimators shrink the overdispersed sample covariance
eigenvalues

As an alternative to both the empirical and the shrinkage estimator we have analyzed a regularization method based on the Cholesky decomposition of the covariance matrixz
