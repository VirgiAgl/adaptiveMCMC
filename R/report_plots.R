...
#plot1<-qplot(iterations, X$X[,1],geom="point", 
#       main="Adaptive MCMC for multidimensional correlated Gaussian distribution", 
#       xlab="Iteration", ylab="", xlim=c(0,n_iter+1), alpha = I(1/1000), size=I(2.5)) +
#       theme(axis.title.x = element_text(size = 15), title = element_text(colour='black'), 
#        axis.text.x=element_text(colour="black"), axis.text.y=element_text(colour="black"), 
#      axis.line = element_line(colour="black", size = 1, linetype = "solid"))+ 
  #        geom_point(size=2, shape=19, alpha=0.1)

#plot2<-qplot(aes(x=iterations, y=X$X[,2]),geom="point", 
#             main="Adaptive MCMC for multidimensional correlated Gaussian distribution", 
#             xlab="Iteration", ylab="", xlim=c(0,100001), alpha = I(1/50), size=I(2.5)) +  
#  theme(axis.title.x = element_text(size = 15), title = element_text(colour='black'), 
#        axis.text.x=element_text(colour="black"), axis.text.y=element_text(colour="black"), 
#        axis.line = element_line(colour="black", size = 1, linetype = "solid"))+ 
...
#plot3<-qplot(x=X$X[,1], 
#             main="Adaptive MCMC for multidimensional correlated Gaussian distribution", 
#             xlab="values", ylab="Frequency", ylim=c(0,100002), alpha = I(1/50), size=I(2.5)) +
#  theme(axis.title.x = element_text(size = 15), title = element_text(colour='black'), 
#        axis.text.x=element_text(colour="black"), axis.text.y=element_text(colour="black"), 
#        axis.line = element_line(colour="black", size = 1, linetype = "solid")) + 
