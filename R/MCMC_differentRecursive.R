...
mcmc = function(target, n_iter = 100, x_1, adapt="None", cov_estimator="Sample covariance", t_adapt = Inf){
  ...
  acceptances = numeric(n_iter+1)  #  Initialise counter for accepted states
  ...
  for (i in 2:(n_iter+1)) {
    ...
    acceptances_values<-acceptances/n_iter
  }  #  End of loop

  cat("\nMCMC finished in", Sys.time() - start, "seconds.")
  cat("\nThe acceptance rate was:", acceptances[((n_iter+1)/n_iter)])
  return(X)
  return(acceptances_values)
}
